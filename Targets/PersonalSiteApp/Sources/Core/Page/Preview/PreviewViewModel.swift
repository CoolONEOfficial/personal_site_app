...
class PreviewViewModel: PreviewViewModeling {
    ...
    private lazy var contentUrl = docUrl.appendingPathComponent("ru")
...
private extension PreviewViewModel {
    ...
    func startServer(outputUrl: URL) {
        ...
        if let url = previewServerService.serverUrl {
            let res = url
                .appendingPathComponent("ru")
                .appendingPathComponent(type.rawValue).appendingPathComponent(pagename)
                .appendingPathComponent("index.html")
            debugPrint("res \(res.absoluteString)")
            self.state = .url(res
            )
        }
    }
